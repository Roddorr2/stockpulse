    ...
    <ProtectedRoute allowedRoles={['ADMIN', 'ENCARGADO_SUCURSAL', 'CAJERO']}>
      <div className="min-h-screen bg-zinc-900 text-zinc-100 flex flex-col font-sans">
        ...
        <main className="flex-1 max-w-7xl w-full mx-auto p-4 md:p-6 space-y-6">
          ...
          {/* Filter Card */}
          <form aria-label="Filtros de búsqueda de ventas" className="bg-zinc-800 border border-zinc-700/80 rounded-xl p-5 flex flex-wrap gap-4 items-end" onSubmit={handleSearch}>
            <div className="flex-1 min-w-[200px] space-y-1.5">
              <label htmlFor="filter-sucursal" className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Sucursal</label>
              <select 
                id="filter-sucursal"
            ...
            <div className="flex-1 min-w-[200px] space-y-1.5">
              <label htmlFor="filter-producto" className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Producto</label>
              <select 
                id="filter-producto"
            ...
            <div className="flex-1 min-w-[200px] space-y-1.5">
              <label htmlFor="filter-fecha-inicio" className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Desde</label>
              <input 
            ...
            <div className="flex-1 min-w-[200px] space-y-1.5">
              <label htmlFor="filter-fecha-fin" className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Hasta</label>
              <input 
                id="filter-fecha-fin"
                type="datetime-local" 
          ...
          <section className="bg-zinc-800 border border-zinc-700/80 rounded-xl overflow-hidden">
            <div className="overflow-x-auto">
              <table aria-label="Historial de ventas registradas" className="w-full text-left text-xs text-zinc-300">
                <thead className="bg-zinc-900 border-b border-zinc-700 text-zinc-400 uppercase font-mono tracking-wider text-[10px]">
                  <tr>
                    <th scope="col" className="py-3 px-5">ID Venta</th>
                    <th scope="col" className="py-3 px-5">Fecha</th>
                    <th scope="col" className="py-3 px-5">Sucursal</th>
                    <th scope="col" className="py-3 px-5">Artículos</th>
                    <th scope="col" className="py-3 px-5">Total</th>
                  </tr>
                </thead>
                  ...
                  {isLoading && sales.length === 0 ? (
                    ...
                    sales.map(venta => (
                      <tr key={venta.id} className="hover:bg-zinc-700/30 transition-colors">
                        ...
                        <td className="py-3 px-5">
                          <ul aria-label={`Artículos de la venta ${venta.id}`} className="space-y-1 text-zinc-400 font-mono text-[11px]">
                            {venta.items?.map(d => (
                              <li key={d.productoId}>
